...
module.exports = async (client, m, myPrefix) => {
   ...
   if (!pc) {
      ...
      console.log('\n' + blueBright.bold('[ LOGS ]'), Func.color(moment(time * 1000).format('DD/MM/YY HH:mm:ss'), 'green'), gray.bgGreen(' ' + m.mtype + ' '), green.bold('from'), '[' + m.sender.split`@` [0] + '] ' + gray.bgYellow(' ' + who + ' '), Func.color('in'), '[' + m.chat + '] ' + gray.bgYellow(' ' + await (await client.groupMetadata(m.chat)).subject + ' '), `\n${Func.mtype(m)}`)
   } else if (pc) {
      ...
      console.log('\n' + blueBright.bold('[ LOGS ]'), Func.color(moment(time * 1000).format('DD/MM/YY HH:mm:ss'), 'green'), gray.bgGreen(' ' + m.mtype + ' '), green.bold('from'), '[' + m.sender.split`@` [0] + '] ' + gray.bgYellow(' ' + who + ' '), Func.color('in'), '[' + m.chat + ']', `\n${Func.mtype(m)}`)
   }
}
